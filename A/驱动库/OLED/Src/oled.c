#include "oled.h"
#include "i2c.h"

#include <stddef.h>
#include <string.h>

#define OLED_BUFFER_SIZE      (OLED_WIDTH * OLED_HEIGHT / 8U)
#define OLED_PAGE_COUNT       (OLED_HEIGHT / 8U)
#define OLED_I2C_TIMEOUT_MS   100U

static uint8_t oled_buffer[OLED_BUFFER_SIZE];
static uint8_t oled_current_x;
static uint8_t oled_current_y;

static const uint8_t oled_font_6x8[][6] = {
  {0x00,0x00,0x00,0x00,0x00,0x00}, /*   */
  {0x00,0x00,0x5F,0x00,0x00,0x00}, /* ! */
  {0x00,0x07,0x00,0x07,0x00,0x00}, /* " */
  {0x14,0x7F,0x14,0x7F,0x14,0x00}, /* # */
  {0x24,0x2A,0x7F,0x2A,0x12,0x00}, /* $ */
  {0x23,0x13,0x08,0x64,0x62,0x00}, /* % */
  {0x36,0x49,0x55,0x22,0x50,0x00}, /* & */
  {0x00,0x05,0x03,0x00,0x00,0x00}, /* ' */
  {0x00,0x1C,0x22,0x41,0x00,0x00}, /* ( */
  {0x00,0x41,0x22,0x1C,0x00,0x00}, /* ) */
  {0x14,0x08,0x3E,0x08,0x14,0x00}, /* * */
  {0x08,0x08,0x3E,0x08,0x08,0x00}, /* + */
  {0x00,0x50,0x30,0x00,0x00,0x00}, /* , */
  {0x08,0x08,0x08,0x08,0x08,0x00}, /* - */
  {0x00,0x60,0x60,0x00,0x00,0x00}, /* . */
  {0x20,0x10,0x08,0x04,0x02,0x00}, /* / */
  {0x3E,0x51,0x49,0x45,0x3E,0x00}, /* 0 */
  {0x00,0x42,0x7F,0x40,0x00,0x00}, /* 1 */
  {0x42,0x61,0x51,0x49,0x46,0x00}, /* 2 */
  {0x21,0x41,0x45,0x4B,0x31,0x00}, /* 3 */
  {0x18,0x14,0x12,0x7F,0x10,0x00}, /* 4 */
  {0x27,0x45,0x45,0x45,0x39,0x00}, /* 5 */
  {0x3C,0x4A,0x49,0x49,0x30,0x00}, /* 6 */
  {0x01,0x71,0x09,0x05,0x03,0x00}, /* 7 */
  {0x36,0x49,0x49,0x49,0x36,0x00}, /* 8 */
  {0x06,0x49,0x49,0x29,0x1E,0x00}, /* 9 */
  {0x00,0x36,0x36,0x00,0x00,0x00}, /* : */
  {0x00,0x56,0x36,0x00,0x00,0x00}, /* ; */
  {0x08,0x14,0x22,0x41,0x00,0x00}, /* < */
  {0x14,0x14,0x14,0x14,0x14,0x00}, /* = */
  {0x00,0x41,0x22,0x14,0x08,0x00}, /* > */
  {0x02,0x01,0x51,0x09,0x06,0x00}, /* ? */
  {0x32,0x49,0x79,0x41,0x3E,0x00}, /* @ */
  {0x7E,0x11,0x11,0x11,0x7E,0x00}, /* A */
  {0x7F,0x49,0x49,0x49,0x36,0x00}, /* B */
  {0x3E,0x41,0x41,0x41,0x22,0x00}, /* C */
  {0x7F,0x41,0x41,0x22,0x1C,0x00}, /* D */
  {0x7F,0x49,0x49,0x49,0x41,0x00}, /* E */
  {0x7F,0x09,0x09,0x09,0x01,0x00}, /* F */
  {0x3E,0x41,0x49,0x49,0x7A,0x00}, /* G */
  {0x7F,0x08,0x08,0x08,0x7F,0x00}, /* H */
  {0x00,0x41,0x7F,0x41,0x00,0x00}, /* I */
  {0x20,0x40,0x41,0x3F,0x01,0x00}, /* J */
  {0x7F,0x08,0x14,0x22,0x41,0x00}, /* K */
  {0x7F,0x40,0x40,0x40,0x40,0x00}, /* L */
  {0x7F,0x02,0x0C,0x02,0x7F,0x00}, /* M */
  {0x7F,0x04,0x08,0x10,0x7F,0x00}, /* N */
  {0x3E,0x41,0x41,0x41,0x3E,0x00}, /* O */
  {0x7F,0x09,0x09,0x09,0x06,0x00}, /* P */
  {0x3E,0x41,0x51,0x21,0x5E,0x00}, /* Q */
  {0x7F,0x09,0x19,0x29,0x46,0x00}, /* R */
  {0x46,0x49,0x49,0x49,0x31,0x00}, /* S */
  {0x01,0x01,0x7F,0x01,0x01,0x00}, /* T */
  {0x3F,0x40,0x40,0x40,0x3F,0x00}, /* U */
  {0x1F,0x20,0x40,0x20,0x1F,0x00}, /* V */
  {0x3F,0x40,0x38,0x40,0x3F,0x00}, /* W */
  {0x63,0x14,0x08,0x14,0x63,0x00}, /* X */
  {0x07,0x08,0x70,0x08,0x07,0x00}, /* Y */
  {0x61,0x51,0x49,0x45,0x43,0x00}, /* Z */
  {0x00,0x7F,0x41,0x41,0x00,0x00}, /* [ */
  {0x02,0x04,0x08,0x10,0x20,0x00}, /* backslash */
  {0x00,0x41,0x41,0x7F,0x00,0x00}, /* ] */
  {0x04,0x02,0x01,0x02,0x04,0x00}, /* ^ */
  {0x40,0x40,0x40,0x40,0x40,0x00}, /* _ */
  {0x00,0x01,0x02,0x04,0x00,0x00}, /* ` */
  {0x20,0x54,0x54,0x54,0x78,0x00}, /* a */
  {0x7F,0x48,0x44,0x44,0x38,0x00}, /* b */
  {0x38,0x44,0x44,0x44,0x20,0x00}, /* c */
  {0x38,0x44,0x44,0x48,0x7F,0x00}, /* d */
  {0x38,0x54,0x54,0x54,0x18,0x00}, /* e */
  {0x08,0x7E,0x09,0x01,0x02,0x00}, /* f */
  {0x0C,0x52,0x52,0x52,0x3E,0x00}, /* g */
  {0x7F,0x08,0x04,0x04,0x78,0x00}, /* h */
  {0x00,0x44,0x7D,0x40,0x00,0x00}, /* i */
  {0x20,0x40,0x44,0x3D,0x00,0x00}, /* j */
  {0x7F,0x10,0x28,0x44,0x00,0x00}, /* k */
  {0x00,0x41,0x7F,0x40,0x00,0x00}, /* l */
  {0x7C,0x04,0x18,0x04,0x78,0x00}, /* m */
  {0x7C,0x08,0x04,0x04,0x78,0x00}, /* n */
  {0x38,0x44,0x44,0x44,0x38,0x00}, /* o */
  {0x7C,0x14,0x14,0x14,0x08,0x00}, /* p */
  {0x08,0x14,0x14,0x18,0x7C,0x00}, /* q */
  {0x7C,0x08,0x04,0x04,0x08,0x00}, /* r */
  {0x48,0x54,0x54,0x54,0x20,0x00}, /* s */
  {0x04,0x3F,0x44,0x40,0x20,0x00}, /* t */
  {0x3C,0x40,0x40,0x20,0x7C,0x00}, /* u */
  {0x1C,0x20,0x40,0x20,0x1C,0x00}, /* v */
  {0x3C,0x40,0x30,0x40,0x3C,0x00}, /* w */
  {0x44,0x28,0x10,0x28,0x44,0x00}, /* x */
  {0x0C,0x50,0x50,0x50,0x3C,0x00}, /* y */
  {0x44,0x64,0x54,0x4C,0x44,0x00}, /* z */
  {0x00,0x08,0x36,0x41,0x00,0x00}, /* { */
  {0x00,0x00,0x7F,0x00,0x00,0x00}, /* | */
  {0x00,0x41,0x36,0x08,0x00,0x00}, /* } */
  {0x08,0x04,0x08,0x10,0x08,0x00}, /* ~ */
};

static HAL_StatusTypeDef OLED_WriteCommand(uint8_t command)
{
  uint8_t data[2] = {0x00U, command};

  return HAL_I2C_Master_Transmit(&hi2c2,
                                 OLED_I2C_ADDRESS,
                                 data,
                                 sizeof(data),
                                 OLED_I2C_TIMEOUT_MS);
}

static HAL_StatusTypeDef OLED_WriteCommands(const uint8_t *commands, size_t length)
{
  HAL_StatusTypeDef status;

  for (size_t i = 0; i < length; i++)
  {
    status = OLED_WriteCommand(commands[i]);
    if (status != HAL_OK)
    {
      return status;
    }
  }

  return HAL_OK;
}

HAL_StatusTypeDef OLED_Init(void)
{
  static const uint8_t init_sequence[] = {
    0xAE,
    0x20, 0x00,
    0xB0,
    0xC8,
    0x00,
    0x10,
    0x40,
    0x81, 0x7F,
    0xA1,
    0xA6,
    0xA8, 0x3F,
    0xA4,
    0xD3, 0x00,
    0xD5, 0x80,
    0xD9, 0xF1,
    0xDA, 0x12,
    0xDB, 0x40,
    0x8D, 0x14,
    0xAF
  };

  HAL_Delay(100);

  HAL_StatusTypeDef status = OLED_WriteCommands(init_sequence, sizeof(init_sequence));
  if (status != HAL_OK)
  {
    return status;
  }

  OLED_Clear();
  return OLED_UpdateScreen();
}

HAL_StatusTypeDef OLED_UpdateScreen(void)
{
  uint8_t page_data[OLED_WIDTH + 1U];
  HAL_StatusTypeDef status;

  page_data[0] = 0x40U;

  for (uint8_t page = 0; page < OLED_PAGE_COUNT; page++)
  {
    status = OLED_WriteCommand((uint8_t)(0xB0U + page));
    if (status != HAL_OK)
    {
      return status;
    }

    status = OLED_WriteCommand(0x00U);
    if (status != HAL_OK)
    {
      return status;
    }

    status = OLED_WriteCommand(0x10U);
    if (status != HAL_OK)
    {
      return status;
    }

    memcpy(&page_data[1], &oled_buffer[OLED_WIDTH * page], OLED_WIDTH);
    status = HAL_I2C_Master_Transmit(&hi2c2,
                                     OLED_I2C_ADDRESS,
                                     page_data,
                                     sizeof(page_data),
                                     OLED_I2C_TIMEOUT_MS);
    if (status != HAL_OK)
    {
      return status;
    }
  }

  return HAL_OK;
}

HAL_StatusTypeDef OLED_DisplayOn(void)
{
  return OLED_WriteCommand(0xAFU);
}

HAL_StatusTypeDef OLED_DisplayOff(void)
{
  return OLED_WriteCommand(0xAEU);
}

HAL_StatusTypeDef OLED_SetContrast(uint8_t contrast)
{
  HAL_StatusTypeDef status = OLED_WriteCommand(0x81U);
  if (status != HAL_OK)
  {
    return status;
  }

  return OLED_WriteCommand(contrast);
}

void OLED_Fill(OLED_Color_t color)
{
  memset(oled_buffer, (color == OLED_COLOR_WHITE) ? 0xFF : 0x00, sizeof(oled_buffer));
}

void OLED_Clear(void)
{
  OLED_Fill(OLED_COLOR_BLACK);
  OLED_SetCursor(0, 0);
}

void OLED_DrawPixel(uint8_t x, uint8_t y, OLED_Color_t color)
{
  if ((x >= OLED_WIDTH) || (y >= OLED_HEIGHT))
  {
    return;
  }

  if (color == OLED_COLOR_WHITE)
  {
    oled_buffer[x + (y / 8U) * OLED_WIDTH] |= (uint8_t)(1U << (y % 8U));
  }
  else
  {
    oled_buffer[x + (y / 8U) * OLED_WIDTH] &= (uint8_t)~(1U << (y % 8U));
  }
}

void OLED_SetCursor(uint8_t x, uint8_t y)
{
  oled_current_x = x;
  oled_current_y = y;
}

void OLED_WriteChar(char ch, OLED_Color_t color)
{
  if ((ch < ' ') || (ch > '~'))
  {
    ch = '?';
  }

  if ((oled_current_x + 6U) > OLED_WIDTH)
  {
    oled_current_x = 0;
    oled_current_y = (uint8_t)(oled_current_y + 8U);
  }

  if ((oled_current_y + 8U) > OLED_HEIGHT)
  {
    return;
  }

  for (uint8_t col = 0; col < 6U; col++)
  {
    uint8_t line = oled_font_6x8[(uint8_t)ch - 32U][col];

    for (uint8_t row = 0; row < 8U; row++)
    {
      OLED_DrawPixel((uint8_t)(oled_current_x + col),
                     (uint8_t)(oled_current_y + row),
                     ((line & (1U << row)) != 0U) ? color : OLED_COLOR_BLACK);
    }
  }

  oled_current_x = (uint8_t)(oled_current_x + 6U);
}

void OLED_WriteString(const char *str, OLED_Color_t color)
{
  if (str == NULL)
  {
    return;
  }

  while (*str != '\0')
  {
    if (*str == '\n')
    {
      oled_current_x = 0;
      oled_current_y = (uint8_t)(oled_current_y + 8U);
    }
    else
    {
      OLED_WriteChar(*str, color);
    }

    str++;
  }
}
